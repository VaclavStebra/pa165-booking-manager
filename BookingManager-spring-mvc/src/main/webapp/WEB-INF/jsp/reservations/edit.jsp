...
<my:pagetemplate>
  <jsp:attribute name="body">
    ...
    <form:form method="post" action="${pageContext.request.contextPath}/${endpoint}/${method}/${reservationEdit.id}" modelAttribute="reservationEdit" cssClass="form-horizontal">
      ...
      <form:select path="user" name="user">
        <c:forEach items="${users}" var="user">
          <option value="${user.id}" ${(user.id == reservationEdit.user.id) ? 'selected' : ''}><c:out value="${user.name}" />&nbsp;<c:out value="${user.surname}" /> &lt;<c:out value="${user.email}" />&gt;</option>
        </c:forEach>
      </form:select>
        </div>
      </div>
      ...
      <form:select path="room" name="room">
        <c:forEach items="${rooms}" var="room">
          <option value="${room.id}" ${(room.id == reservationEdit.room.id) ? 'selected' : ''}><c:out value="${room.roomIdentifier}" /></option>
        </c:forEach>
      </form:select>
        </div>
      </div>
